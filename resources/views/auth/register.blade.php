<x-layouts.guest>
    <h2 class="mb-1 text-center font-display text-2xl font-bold text-primary-800 dark:text-sand-100">{{ __('auth.register') }}</h2>
    <p class="mb-6 text-center text-sm text-ink-900/60 dark:text-sand-100/60">{{ __('auth.register_subtitle') }}</p>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        @csrf
        <input type="hidden" name="timezone" id="tz-field">
        <div>
            <label class="label" for="name">{{ __('auth.name') }}</label>
            <input id="name" name="name" type="text" value="{{ old('name') }}" required autofocus class="field" placeholder="{{ __('auth.name') }}">
            @error('name') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="label" for="email">{{ __('auth.email') }}</label>
            <input id="email" name="email" type="email" value="{{ old('email') }}" required class="field" dir="ltr" placeholder="you@example.com">
        ...
        <button type="submit" class="btn-primary w-full">{{ __('auth.register') }}</button>
        <p class="text-center text-xs text-ink-900/50 dark:text-sand-100/50">{{ __('auth.agree_terms') }}</p>
    </form>
    ...
    <script>
        // Capture the browser timezone so prayer times are right from day one.
